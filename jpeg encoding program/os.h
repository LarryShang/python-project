...
//-------Use this file as-is from Programming Assignment 4---------
...
// We assume that there is only one writer task per FIFO
	...
	void write(const T& data) {
		//-----REUSE CODE FROM PROG. ASSIGNMENT 4--------
		...
		os_task *temp_t = OS->CURRENT;
				...
				if( blocked_reader != NULL){
					if(temp_t->get_priority() < blocked_reader->get_priority()){
...
// We assume that there is only one reader task per FIFO
	void read(T& data) {
		...
		if( blocked_writer != NULL){
			if(temp_t->get_priority() < blocked_writer->get_priority()){
